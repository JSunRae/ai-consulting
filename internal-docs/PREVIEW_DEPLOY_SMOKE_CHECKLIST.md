...
- Local preview-style smoke validation is already recorded in `internal-docs/RELEASE_SMOKE_TEST_REPORT.md`.
- The remaining unresolved gate is the hosted preview URL and deploy metadata.
- Do not mark this checklist complete until the actual Netlify preview is available and the unmatched-route 404 behavior is confirmed on that host.
...
- [ ] Legacy route redirects immediately to `/blog/5-ways-llm-workflows-transform-business-analytics.html`
- [ ] Canonical target article loads cleanly after redirect
- [ ] No redirect loop or browser error appears
...
- [ ] Social archive page remains readable
  - Not re-run on the deploy permalink in this pass. Prior branch-local browser QA already covered mobile behavior in `internal-docs/BROWSER_QA_REPORT.md`.
